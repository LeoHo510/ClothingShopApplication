...
import com.example.appuser.zalo.Helper.Helpers;
import com.example.appuser.zalo.HttpProvider;
     ...
     public JSONObject createOrder(String amount) throws Exception {
        ...
        JSONObject data = HttpProvider.sendPost(AppInfo.URL_CREATE_ORDER, formBody);
         ...
         for (int i = 0; i < ((FormBody) formBody).size(); i++) {
             jsonBody.put(((FormBody) formBody).name(i), ((FormBody) formBody).value(i));
         }
         Log.d("FormBodyZalo", "Request Body: " + jsonBody.toString());
